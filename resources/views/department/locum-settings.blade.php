        ...
        <div class="content container-fluid">
            <div class="page-header mb-4">
                <div class="row align-items-center">
                    <div class="col">
                        <h6 class="page-title mb-2 text-muted">Locum Approval Settings</h6>
                    </div>
                    <div class="col-auto">
            ...
            <div class="alert alert-info border-0 shadow-sm">
                ...
                <ul class="mb-0 ps-3">
                    ...
                    <li><em>In-Charge + Platform</em> routes: <strong>In-Charge → Platform Manager(s) → Line Manager →
                            HR</strong>.</li>
                    <li><em>In-Charge + LM + HEC</em> routes: <strong>In-Charge → Line Manager → HEC → HR</strong>.</li>
                </ul>
            </div>
            {{-- ========================================= --}}
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="locumTable" class="table table-hover table-bordered align-middle">
                            ...
                            <tbody>
                                @forelse ($departments as $department)
                                    @php
                                        $isStandard =
                                            !$department->has_three_level_approval &&
                                            !$department->has_incharge_platform_flow &&
                                            !($department->has_incharge_lm_hec_flow ?? false);
                                        $is3Level = (bool) $department->has_three_level_approval;
                                        $isPlatFlow = (bool) $department->has_incharge_platform_flow;
                                        $isILHH = (bool) ($department->has_incharge_lm_hec_flow ?? false);

                                        $currentText = $isPlatFlow
                                            ? 'In-Charge → Platform Manager(s) → Line Manager → HR'
                                            : ($isILHH
                                                ? 'In-Charge → Line Manager → HEC → HR'
                                                : ($is3Level
                                                    ? 'Line Manager → HEC → HR'
                                                    : 'Line Manager → HR'));
                                    @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $currentText }}</td>
                                        <td>
                                            <form method="POST" action="{{ route('departments.update-locum-settings') }}"
                                                ...
                                                <select name="approval_flow" class="form-select form-select-sm flow-select">
                                                    ...
                                                    <option value="incharge_platform" {{ $isPlatFlow ? 'selected' : '' }}>
                                                        In-Charge → Platform Manager(s) → Line Manager → HR
                                                    </option>
                                                    <option value="incharge_lm_hec_hr" {{ $isILHH ? 'selected' : '' }}>
                                                        In-Charge → Line Manager → HEC → HR
                                                    </option>
                                                </select>
