...
<head>
    ...
    <title>Certificate of Service Delivery</title>
    <style>
        ...
        .note-box{background-color:#fff3cd;border:1px solid #ffe69c;border-left:4px solid #ffc107;border-radius:6px;padding:15px;margin:20px 0;color:#856404}
...
<body>
    <div class="email-wrapper">
        <div class="header">
            <h1>📄 Certificate of Service Ready</h1>
        </div>

        <div class="content">
            <div class="greeting">
                <p>Dear <strong>{{ $recipientName ?? $staffName }}</strong>,</p>
            </div>

            <div class="status-banner">
                <span class="status-icon">✅</span>
                <strong>Your Certificate of Service has been prepared and attached to this email</strong>
            </div>

            <p>
                Please find attached the <strong>Certificate of Service</strong> for <strong>{{ $staffName }}</strong>. This document confirms the employment details recorded by the Human Resources department.
            </p>
            ...
            <div class="info-card">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $certificate->date_of_joining?->format('d F Y') ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Last Working Day:</span>
                    <span class="info-value">{{ $certificate->last_working_day?->format('d F Y') ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Issue Date:</span>
                    <span class="info-value">{{ $certificate->issue_date?->format('d F Y') ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
            ...
            <div class="note-box">
                The certificate PDF is attached to this email. Please keep it for your records. If the intended staff recipient has no personal email on file, this email may be sent to the HR team mailbox for follow-up delivery.
            </div>
