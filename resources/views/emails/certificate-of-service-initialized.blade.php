...
<head>
  ...
  <title>Certificate of Service – Action Required</title>
  <style>
...
<body>
  <div class="email-wrapper">
    <div class="header">
      <h1>📄 Certificate of Service – Action Required</h1>
    </div>

    <div class="content">
      @php
        $staffName = trim(($certificate->staff?->fname ?? '') . ' ' . ($certificate->staff?->lname ?? ''));
        $initName  = trim(($initializer->fname ?? '') . ' ' . ($initializer->lname ?? '')) ?: ($initializer->username ?? 'HR Officer');
      @endphp

      <div class="greeting">
        <p>Dear <strong>COO</strong>,</p>
        <p>HR has initialized a Certificate of Service for a staff member and it is now pending your review and signature.</p>
      </div>

      <div class="status-banner">
        <span class="status-icon">📋</span>
        <strong>Certificate of Service Awaiting COO Review</strong>
      </div>
      ...
      <div class="info-card">
        ...
        @if ($certificate->staff?->ccbrt_code)
        <div class="info-row">
          <span class="info-label">Staff Code:</span>
          <span class="info-value">{{ $certificate->staff->ccbrt_code }}</span>
        </div>
        @endif
        <div class="info-row">
          <span class="info-label">Position Held:</span>
          <span class="info-value">{{ $certificate->position_held }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Department:</span>
          <span class="info-value">{{ $certificate->department }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Date of Joining:</span>
          <span class="info-value">{{ $certificate->date_of_joining?->format('d F Y') }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Last Working Day:</span>
          <span class="info-value">{{ $certificate->last_working_day?->format('d F Y') }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Issue Date:</span>
          <span class="info-value">{{ $certificate->issue_date?->format('d F Y') }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Initialized By:</span>
          <span class="info-value">{{ $initName }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Initialized On:</span>
          <span class="info-value">{{ $certificate->initialized_at?->format('d F Y, H:i') }}</span>
        </div>
      </div>

      <div class="notice-box">
        <strong>⚠ Action Required:</strong> Please review the certificate details in e-Docs and confirm it is ready for issuance.
      </div>

      <div class="button-container">
        <a href="{{ $viewUrl }}" class="action-button" target="_blank">
          View Certificate in e-Docs
        </a>
      </div>
