...
<head>
    ...
    <title>On-Call Claim Status Update</title>
    <style>
        body {
...
<body>
    <div class="email-wrapper">
        <div class="header">
            <h1>📋 On-Call Claim Status Update</h1>
        </div>
        ...
        <div class="content">
            ...
            <div class="status-banner {{ $status === 'approved' ? 'status-approved' : 'status-rejected' }}">
                @if ($status === 'approved')
                    ...
                    <strong>Your on-call claim has been APPROVED{{ $approvalStep ? ' by ' . $approvalStep : '' }}</strong>
                @else
                    <span class="status-icon">❌</span>
