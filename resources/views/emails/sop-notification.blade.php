...
<head>
  ...
  <title>{{ $isNew ? 'New' : 'Updated' }} SOP</title>
  <style>
...
<body>
  <div class="email-wrapper">
    <div class="header">
      <h1>📄 {{ $isNew ? 'New' : 'Updated' }} Standard Operating Procedure</h1>
    </div>
    ...
    <div class="content">
      ...
      <div class="greeting">
        ...
        <p>{{ $isNew ? 'A new Standard Operating Procedure has been added for your department and is available to view.' : 'A Standard Operating Procedure in your department has been updated.' }}</p>
      </div>
      ...
      <div class="info-card">
        ...
        <div class="info-row">
          ...
          <span class="info-value">{{ $sop->title }}</span>
        </div>
        @if ($sop->document_code)
        <div class="info-row">
          <span class="info-label">Document code:</span>
          <span class="info-value">{{ $sop->document_code }}</span>
        </div>
        @endif
        <div class="info-row">
          <span class="info-label">Owner Department:</span>
          <span class="info-value">{{ $sop->ownerDepartment->dept_name ?? 'N/A' }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Effective:</span>
          <span class="info-value">{{ $sop->effective_date ? $sop->effective_date->format('d M Y') : 'N/A' }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Expiry:</span>
          <span class="info-value">{{ $sop->expiry_date ? $sop->expiry_date->format('d M Y') : 'N/A' }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">{{ $isNew ? 'Published' : 'Updated' }}:</span>
          <span class="info-value">{{ $isNew ? ($sop->created_at?->format('d F Y, H:i') ?? 'N/A') : ($sop->updated_at?->format('d F Y, H:i') ?? 'N/A') }}</span>
        </div>
      </div>
