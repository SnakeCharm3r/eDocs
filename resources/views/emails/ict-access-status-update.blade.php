...
<head>
  ...
  <title>ICT Access Request Status Update</title>
  <style>
...
<body>
  <div class="email-wrapper">
    <div class="header">
      <h1>💻 ICT Access Request Status Update</h1>
    </div>

    <div class="content">
      @php
        $requester = $submitter ?? $ictAccessResource->user ?? null;
      ...
      <div class="status-banner {{ $status === 'approved' ? 'status-approved' : 'status-rejected' }}">
        @if ($status === 'approved')
          ...
          <strong>Your ICT Access Request has been APPROVED{{ $approvalStep ? ' by ' . $approvalStep : '' }}</strong>
        @else
          <span class="status-icon">❌</span>
          <strong>Your ICT Access Request has been REJECTED</strong>
        @endif
      </div>
      ...
      <div class="info-card">
        ...
        <div class="info-row">
          ...
          <span class="info-value">#{{ $ictAccessResource->id }}</span>
        </div>
        <div class="info-row">
        ...
        <div class="info-row">
          ...
          <span class="info-value">{{ $ictAccessResource->user_type ?? 'N/A' }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Access Action:</span>
          <span class="info-value">{{ $ictAccessResource->access_required ?? 'N/A' }}</span>
        </div>
        @if ($approver)
        <div class="info-row">
        ...
        <div class="info-row">
          ...
          <span class="info-value">{{ $ictAccessResource->created_at ? \Carbon\Carbon::parse($ictAccessResource->created_at)->format('d F Y, H:i') : 'N/A' }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Status Update Date:</span>
