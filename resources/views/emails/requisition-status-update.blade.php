...
<head>
    ...
    <title>Requisition Request Status Update</title>
    <style>
        body {
...
<body>
    <div class="email-wrapper">
        ...
        <div class="content">
            @php
                $submitterName = trim(($submitter->fname ?? '').' '.($submitter->lname ?? ''));
                if ($submitterName === '') {
                    $submitterName = $submitter->username ?? 'Requester';
                }
                ...
                $isRejected = in_array($status, ['rejected', 'rejected_for_editing']);
                $isPending = !$isApproved && !$isRejected;
                
                // Status class
                $statusClass = $isApproved ? 'status-approved' : ($isRejected ? 'status-rejected' : 'status-pending');
            @endphp
            ...
            <div class="status-banner {{ $statusClass }}">
                @if ($isApproved)
                    ...
                    <strong>Your Requisition has been APPROVED{{ $approvalStep ? ' at ' . $approvalStep : '' }}</strong>
                @elseif ($isRejected)
                    <span class="status-icon">❌</span>
                    <strong>Your Requisition has been REJECTED{{ $approvalStep ? ' at ' . $approvalStep : '' }}</strong>
                @else
                    <span class="status-icon">⏳</span>
                    <strong>Requisition Status Update{{ $approvalStep ? ' - ' . $approvalStep : '' }}</strong>
                @endif
            </div>
            ...
            <div class="info-card">
                ...
                @if ($approvalStep)
                <div class="info-row">
                    <span class="info-label">Approval Stage:</span>
