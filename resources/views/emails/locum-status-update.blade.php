...
<head>
    ...
    <title>Locum Claim Status Update</title>
    <style>
        body {
...
<body>
    <div class="email-wrapper">
        <div class="header">
            <h1>📋 Locum Claim Status Update</h1>
        </div>

        <div class="content">
            @php
                $requester = $locumRequest->user ?? null;
                ...
                $year = optional($locumRequest->created_at)->format('Y');
                if (property_exists($locumRequest, 'locum_year') && !empty($locumRequest->locum_year)) {
                    $year = $locumRequest->locum_year;
                }
            @endphp
            ...
            <div class="status-banner {{ $status === 'approved' ? 'status-approved' : 'status-rejected' }}">
                @if ($status === 'approved')
                    ...
                    <strong>Your locum claim has been APPROVED</strong>
                @else
                    <span class="status-icon">❌</span>
                    <strong>Your locum claim has been REJECTED</strong>
                @endif
            </div>
