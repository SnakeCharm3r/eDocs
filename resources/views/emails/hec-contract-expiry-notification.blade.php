...
<head>
  ...
  <title>{{ $type === 'expired' ? 'HEC Contract Expired' : 'HEC Contract Nearing Expiry' }}</title>
  <style>
    ...
    <div class="content">
      <div class="greeting">
        @if(isset($recipient) && $recipient)
          ...
          @php $recipientRole = ucfirst(str_replace('-', ' ', $recipient->getRoleNames()->first() ?? '')); @endphp
          @if($recipientRole)<p style="font-size:13px;color:#666;margin-top:-10px;">{{ $recipientRole }}</p>@endif
        @else
          <p>Dear Team,</p>
        @endif
        @if ($type === 'expired')
          <p>This is a reminder that the following HEC contract has <strong>expired</strong> and no action has been taken for at least 2 days. Please review and take the necessary steps (renewal, termination, or closure).</p>
        @else
          <p>This is a reminder that the following HEC contract is <strong>nearing its end date</strong>. Please review and take any necessary actions (renewal, termination, etc.).</p>
        @endif
      </div>

      @if ($type === 'expired')
        <div class="alert-expired">
          <strong>Expired (no action in 2+ days)</strong> – Please update or close this contract in e-Docs.
        </div>
      @else
        <div class="alert-near">
          <strong>Nearing expiry</strong> – Please review and take action before the end date.
        </div>
      @endif
      ...
      <div class="info-card">
        ...
        <div class="info-row">
          ...
          <span class="info-value">{{ $contract->contract_number ?? 'N/A' }}</span>
        </div>
        <div class="info-row">
        ...
        @if ($type === 'near_expiry' && $contract->end_date)
          <div class="info-row">
            <span class="info-label">Days remaining:</span>
            <span class="info-value">{{ \Carbon\Carbon::today()->diffInDays($contract->end_date, false) }}</span>
          </div>
        @endif
        <div class="info-row">
          <span class="info-label">Status:</span>
          <span class="info-value">{{ $contract->status ?? 'N/A' }}</span>
        </div>
      </div>

      <div class="button-container">
        <a href="{{ $viewUrl }}" class="action-button" target="_blank">
          View HEC Contracts in e-Docs
        </a>
      </div>
