        ...
        <div class="content container-fluid">
            <div class="page-header">
                <div class="row align-items-center">
                    <div class="col">
                        <h3 class="page-title">
                            <i class="fas fa-edit me-2"></i>Edit HEC Contract
                        </h3>
                    </div>
                    <div class="col-auto">
                    ...
                    <div class="card">
                        <div class="card-header bg-light">
                            <h5 class="card-title mb-0">
                                <i class="fas fa-file-contract me-2"></i>Edit HEC Contract: {{ $contract->title }}
                            </h5>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('hec-contracts.update', $contract->id) }}" method="POST"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select @error('contract_source') is-invalid @enderror"
                                                ...
                                                <option value="new"
                                                    {{ old('contract_source', $contract->contract_source ?? 'new') == 'new' ? 'selected' : '' }}>
                                                    New
                                                    Contract</option>
                                                <option value="existing"
                                                    {{ old('contract_source', $contract->contract_source) == 'existing' ? 'selected' : '' }}>
                                                    Existing
                                                    Contract</option>
                                            </select>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select @error('status') is-invalid @enderror" id="status"
                                                ...
                                                <option value="draft"
                                                    {{ old('status', $contract->status) == 'draft' ? 'selected' : '' }}>
                                                    Draft</option>
                                                <option value="active"
                                                    {{ old('status', $contract->status) == 'active' ? 'selected' : '' }}>
                                                    Active</option>
                                                <option value="in_progress"
                                                    {{ old('status', $contract->status) == 'in_progress' ? 'selected' : '' }}>
                                                    In Progress</option>
                                                <option value="renewed"
                                                    {{ old('status', $contract->status) == 'renewed' ? 'selected' : '' }}>
                                                    Renewed</option>
                                                ...
                                                <option value="soonToExpire"
                                                    {{ old('status', $contract->status) == 'soonToExpire' ? 'selected' : '' }}>
                                                    Soon To Expire
                                                </option>
                                            </select>
                                            @error('status')
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select @error('contract_owner_id') is-invalid @enderror"
                                                ...
                                                @forelse ($hecMembers ?? [] as $member)
                                                    <option value="{{ $member->id }}"
                                                        {{ old('contract_owner_id', $contract->contract_owner_id) == $member->id ? 'selected' : '' }}>
                                            ...
                                            <small class="form-text text-muted">
                                                Select a HEC member (COO, CFO, CMS, CCDRO). If none, leave blank and fill
                                                the email below.
                                            </small>
                                        ...
                                        <div class="col-md-6 mb-3" id="owner_email_container" style="display: none;">
                                            ...
                                            <small class="form-text text-muted">
                                                Required when no HEC member is selected. This email will receive expiry /
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="text"
                                                ...
                                                value="{{ old('cost', $contract->cost ? number_format($contract->cost, 2, '.', ',') : '') }}"
                                                required>
                                            ...
                                            <select class="form-select @error('currency') is-invalid @enderror"
                                                ...
                                                @foreach ($currencyOptions as $currencyOption)
                                                    <option value="{{ $currencyOption }}"
                                                        {{ old('currency', $contract->currency ?? 'TZS') == $currencyOption ? 'selected' : '' }}>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <small class="form-text text-muted">Max: 10MB (Leave empty to keep current
                                                file)</small>
                                            @error('file_path')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            @if ($contract->signed_contract_path)
                                                <div class="mb-2">
                                                    <a href="{{ asset('storage/' . $contract->signed_contract_path) }}"
                                                        target="_blank" class="btn btn-sm btn-outline-primary">
                                                        <i class="fas fa-file-pdf me-1"></i> View Current
                                                    </a>
                                                </div>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            @if ($contract->terms_conditions_path)
                                                <div class="mb-2">
                                                    <a href="{{ asset('storage/' . $contract->terms_conditions_path) }}"
                                                        target="_blank" class="btn btn-sm btn-outline-primary">
                                                        <i class="fas fa-file-pdf me-1"></i> View Current
                                                    </a>
                                                </div>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            @if ($contract->sla_document_path)
                                                <div class="mb-2">
                                                    <a href="{{ asset('storage/' . $contract->sla_document_path) }}"
                                                        target="_blank" class="btn btn-sm btn-outline-primary">
                                                        <i class="fas fa-file-pdf me-1"></i> View Current
                                                    </a>
                                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select
                                                ...
                                                <option value="Low"
                                                    {{ old('impact_if_not_requested', $contract->impact_if_not_requested) == 'Low' ? 'selected' : '' }}>
                                                    Low
                                                </option>
                                                <option value="Medium"
                                                    {{ old('impact_if_not_requested', $contract->impact_if_not_requested) == 'Medium' ? 'selected' : '' }}>
                                                    Medium</option>
                                                <option value="High"
                                                    {{ old('impact_if_not_requested', $contract->impact_if_not_requested) == 'High' ? 'selected' : '' }}>
                                                    High
                                                </option>
                                            </select>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select @error('likelihood_rating') is-invalid @enderror"
                                                ...
                                                <option value="Low"
                                                    {{ old('likelihood_rating', $contract->likelihood_rating) == 'Low' ? 'selected' : '' }}>
                                                    Low</option>
                                                <option value="Medium"
                                                    {{ old('likelihood_rating', $contract->likelihood_rating) == 'Medium' ? 'selected' : '' }}>
                                                    Medium
                                                </option>
                                                <option value="High"
                                                    {{ old('likelihood_rating', $contract->likelihood_rating) == 'High' ? 'selected' : '' }}>
                                                    High
                                                </option>
                                            </select>
                                ...
                                <div class="mt-4">
                                    <button type="submit" class="btn btn-success">
                                        <i class="fas fa-save me-1"></i> Update HEC Contract
                                    </button>
                                    <a href="{{ route('hec-contracts.index') }}" class="btn btn-secondary">
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Initial check on page load (check if contract has owner_email but no contract_owner_id)
                if (contractOwnerSelect.value === '' && ownerEmailInput.value) {
                    ownerEmailContainer.style.display = 'block';
                    ownerEmailInput.setAttribute('required', 'required');
                } else {
                    toggleOwnerEmail();
