    ...
    <div class="content container-fluid" style="background-color: #eff8f3;">
                ...
                <div class="col-sm-12">
                    <div class="page-sub-header d-flex justify-content-between align-items-center">
                        <h3 class="page-title mb-0">
                            <i class="fas fa-heartbeat me-2"></i>Health Details - {{ $user->fname }} {{ $user->lname }}
                        </h3>
        ...
        <div class="container my-5">
            <div class="alert alert-info mb-4">
                ...
                <strong>HR Mode:</strong> You are adding health details for <strong>{{ $user->fname }} {{ $user->lname }}</strong>.
                <br><small>Please provide health information. All fields marked with <span class="text-danger">*</span> are required.</small>
            </div>
            ...
            {{-- Health Details Form --}}
            <form action="{{ route('hr.employee.save-health-details', $user->id) }}" method="POST" id="healthDetailsForm">
                @csrf
                    ...
                    <div class="col-12 col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('physical_disability') is-invalid @enderror"
                                ...
                                <option value="None"
                                    {{ old('physical_disability', ($healthDetail->physical_disability ?? $healthDetails->physical_disability ?? '')) == 'None' ? 'selected' : '' }}>
                                    None</option>
                                <option value="Wheelchair-bound"
                                    {{ old('physical_disability', ($healthDetail->physical_disability ?? $healthDetails->physical_disability ?? '')) == 'Wheelchair-bound' ? 'selected' : '' }}>
                                    Wheelchair-bound</option>
                                <option value="Visually Impaired"
                                    {{ old('physical_disability', ($healthDetail->physical_disability ?? $healthDetails->physical_disability ?? '')) == 'Visually Impaired' ? 'selected' : '' }}>
                                    Visually Impaired</option>
                                <option value="Hearing Impaired"
                                    {{ old('physical_disability', ($healthDetail->physical_disability ?? $healthDetails->physical_disability ?? '')) == 'Hearing Impaired' ? 'selected' : '' }}>
                                    Hearing Impaired</option>
                                <option value="Other"
                                    {{ old('physical_disability', ($healthDetail->physical_disability ?? $healthDetails->physical_disability ?? '')) == 'Other' ? 'selected' : '' }}>
                                    Other</option>
                            </select>
                        ...
                        @php
                            ...
                            $currentDisability = old('physical_disability', ($healthDetail->physical_disability ?? $healthDetails->physical_disability ?? ''));
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="blood_group" id="blood_group">
                                ...
                                <option value="A+"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'A+' ? 'selected' : '' }}>A+
                                </option>
                                <option value="A-"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'A-' ? 'selected' : '' }}>A-
                                </option>
                                <option value="B+"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'B+' ? 'selected' : '' }}>B+
                                </option>
                                <option value="B-"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'B-' ? 'selected' : '' }}>B-
                                </option>
                                <option value="AB+"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'AB+' ? 'selected' : '' }}>
                                    AB+</option>
                                <option value="AB-"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'AB-' ? 'selected' : '' }}>
                                    AB-</option>
                                <option value="O+"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'O+' ? 'selected' : '' }}>O+
                                </option>
                                <option value="O-"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'O-' ? 'selected' : '' }}>O-
                                </option>
                                <option value="unknown"
                                    {{ old('blood_group', ($healthDetail->blood_group ?? $healthDetails->blood_group ?? '')) == 'unknown' ? 'selected' : '' }}>
                                    I don't know</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="illness_history" id="illness_history" rows="4"
                                placeholder="E.g., Asthma, Diabetes, Hypertension">{{ old('illness_history', ($healthDetail->illness_history ?? $healthDetails->illness_history ?? '')) }}</textarea>
                    ...
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="allergies" id="allergies" rows="4"
                                placeholder="E.g., Penicillin, Peanuts, Shellfish">{{ old('allergies', ($healthDetail->allergies ?? $healthDetails->allergies ?? '')) }}</textarea>
                        ...
                        <div class="form-group" id="insuranceNameGroup" style="display: none;">
                            ...
                            <input type="hidden" name="insur_name" id="insur_name"
                                value="{{ old('insur_name', ($healthDetail->insur_name ?? $healthDetails->insur_name ?? '')) }}">
                        ...
                        <div class="form-group" id="insuranceNumberGroup" style="display: none;">
                            ...
                            <input type="text" class="form-control @error('insur_no') is-invalid @enderror" name="insur_no" id="insur_no"
                                value="{{ old('insur_no', ($healthDetail->insur_no ?? $healthDetails->insur_no ?? '')) }}"
