    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        <div>
            <h4 class="mb-1" style="color: #333; font-weight: 300;">
                <i class="fas fa-user-plus me-2" style="color: #007A33;"></i>
                New Recruitment Requisition
            </h4>
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                ...
                <div class="mb-4">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="pos-type-card w-100 {{ old('position_type') == 'new_position' ? 'selected' : '' }}" for="new_position">
                                <input type="radio" name="position_type" id="new_position" value="new_position"
                                       {{ old('position_type') == 'new_position' ? 'checked' : '' }}>
                        ...
                        <div class="col-md-4">
                            <label class="pos-type-card w-100 {{ old('position_type') == 'replacement' ? 'selected' : '' }}" for="replacement">
                                <input type="radio" name="position_type" id="replacement" value="replacement"
                                       {{ old('position_type') == 'replacement' ? 'checked' : '' }}>
                                <div class="pos-type-icon">
                        ...
                        <div class="col-md-4">
                            <label class="pos-type-card w-100 {{ old('position_type') == 'contract_renewal' ? 'selected' : '' }}" for="contract_renewal">
                                <input type="radio" name="position_type" id="contract_renewal" value="contract_renewal"
                                       {{ old('position_type') == 'contract_renewal' ? 'checked' : '' }}>
                                <div class="pos-type-icon">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="responsibility_centre" id="responsibility_centre"
                               class="form-control form-control-sm" value="{{ old('responsibility_centre') }}"
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="reporting_line" id="reporting_line" class="form-control form-control-sm"
                               value="{{ old('reporting_line') }}" placeholder="e.g., Head of Department">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="required_start_date" id="required_start_date"
                               class="form-control form-control-sm" value="{{ old('required_start_date') }}"
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="field-hint">PDF only, max 10MB</div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex gap-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="employee_count_mode" id="mode_single" value="single" checked>
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="date" name="current_contract_end_date" id="rep_contract_end_date"
                                       class="form-control form-control-sm" value="{{ old('current_contract_end_date') }}">
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="job_title_id" id="job_title_id" class="form-select form-select-sm">
                                    ...
                                    @foreach($jobTitles as $jt)
                                        <option value="{{ $jt->id }}" {{ old('job_title_id') == $jt->id ? 'selected' : '' }}>{{ $jt->job_title }}</option>
                                    @endforeach
                                </select>
                                @error('job_title_id')
                                    <small class="text-danger d-block">{{ $message }}</small>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="contract_type" id="contract_type" class="form-select form-select-sm">
                                    ...
                                    <option value="minimal_1_year"     {{ old('contract_type') == 'minimal_1_year'     ? 'selected' : '' }}>Minimal 1 year</option>
                                    <option value="termed_less_1_year" {{ old('contract_type') == 'termed_less_1_year' ? 'selected' : '' }}>Termed &lt; 1 year</option>
                                    <option value="health_volunteer"   {{ old('contract_type') == 'health_volunteer'   ? 'selected' : '' }}>Health Volunteer</option>
                                    <option value="work_exposure"      {{ old('contract_type') == 'work_exposure'      ? 'selected' : '' }}>Work Exposure</option>
                                </select>
                                @error('contract_type')
                                    <small class="text-danger d-block">{{ $message }}</small>
                                @enderror
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="new_job_title" id="new_job_title"
                                       class="form-control form-control-sm" value="{{ old('new_job_title') }}"
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="contract_type" id="contract_type_new" class="form-select form-select-sm">
                                    <option value="">— Select —</option>
                                    <option value="minimal_1_year">Minimal 1 year (employment)</option>
                                    <option value="termed_less_1_year">Termed &lt; 1 year (consultant/task)</option>
                                    <option value="health_volunteer">Health Volunteer (50% basic)</option>
                                    <option value="work_exposure">Work Exposure Placement (no pay)</option>
                                </select>
                                @error('contract_type')
                                    <small class="text-danger d-block">{{ $message }}</small>
                                @enderror
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                ...
                <div class="row g-2 mb-4">
                    <div class="col-md-6">
                        <label class="cond-item w-100 {{ old('condition_medical_operational') ? 'checked' : '' }}" for="cond1">
                            <input type="checkbox" name="condition_medical_operational" id="cond1"
                                   class="cond-check" value="1" {{ old('condition_medical_operational') ? 'checked' : '' }}>
                            <div>
                    ...
                    <div class="col-md-6">
                        <label class="cond-item w-100 {{ old('condition_safety_reputational') ? 'checked' : '' }}" for="cond2">
                            <input type="checkbox" name="condition_safety_reputational" id="cond2"
                                   class="cond-check" value="1" {{ old('condition_safety_reputational') ? 'checked' : '' }}>
                            <div>
                    ...
                    <div class="col-md-6">
                        <label class="cond-item w-100 {{ old('condition_legal_requirement') ? 'checked' : '' }}" for="cond3">
                            <input type="checkbox" name="condition_legal_requirement" id="cond3"
                                   class="cond-check" value="1" {{ old('condition_legal_requirement') ? 'checked' : '' }}>
                            <div>
                    ...
                    <div class="col-md-6">
                        <label class="cond-item w-100 {{ old('condition_financial_loss') ? 'checked' : '' }}" for="cond4">
                            <input type="checkbox" name="condition_financial_loss" id="cond4"
                                   class="cond-check" value="1" {{ old('condition_financial_loss') ? 'checked' : '' }}>
                            <div>
                    ...
                    <div class="col-md-6">
                        <label class="cond-item w-100 {{ old('condition_increase_income') ? 'checked' : '' }}" for="cond5">
                            <input type="checkbox" name="condition_increase_income" id="cond5"
                                   class="cond-check" value="1" {{ old('condition_increase_income') ? 'checked' : '' }}>
                            <div>
                ...
                <textarea name="elaborate_reason" id="elaborate_reason" class="form-control" rows="4"
                          ...
                          required minlength="50">{{ old('elaborate_reason') }}</textarea>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (repEmpSel) initSelect2El(repEmpSel, 'Search employee to replace...', false);
    ...
    function loadEmployeesForDept(deptId, posType) {
        const includeInactive = (posType === 'contract_renewal' || posType === 'replacement') ? '&include_inactive=1' : '';
        fetch(`{{ route('requisitions.employees') }}?department_id=${deptId}${includeInactive}`)
            ...
            .then(data => {
                ...
                if (repEmpSel) {
                    $('#employee_id').empty();
                    employees.forEach(e => $('#employee_id').append(new Option(e.name, e.id, false, false)));
                    $('#employee_id').trigger('change');
                }
                // Populate renewal select
                if (renEmpSel) {
                    $('#renewal_employee_id').empty();
                    employees.forEach(e => $('#renewal_employee_id').append(new Option(e.name + (e.status_label||''), e.id, false, false)));
    ...
    if (finalDeptId) {
        loadJobTitlesByDepartment(finalDeptId);
        loadEmployeesForDept(finalDeptId, 'replacement');
    }
    @endif
});

function loadJobTitlesByDepartment(deptId) {
    if (!deptId) return;
