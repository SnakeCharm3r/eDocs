...
class OnCallRequestsExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithColumnFormatting, WithEvents
{
    ...
    public function map($req): array
    {
        ...
        [$claimMonthNum, $claimYearNum] = self::parseClaimToMonthYear($req->locum_month, $req->locum_year, $req->created_at);
    ...
    {
        // Hours is column I and Amount is J.
        return [
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $items = $this->rows->map(function ($req) {
                    [$cm, $cy] = self::parseClaimToMonthYear($req->locum_month, $req->locum_year, $req->created_at);
                    if (!$cm || !$cy) {
    ...
    /**
     * Parse "locum_month" and "locum_year" into (month, year)
     * Accepts:
     *  - "September 2025" or "September" with separate year
     *  - Best-effort Carbon parse fallback
     */
    public static function parseClaimToMonthYear(?string $monthText, ?int $year, $createdAt): array
    {
        $created = $createdAt ? Carbon::parse($createdAt) : null;
        $fallbackYear = $year ?? ($created ? (int)$created->year : (int)date('Y'));

        if (!$monthText || !is_string($monthText)) {
            return [$created ? (int)$created->month : null, $fallbackYear];
        }
